...
filter_data_question = FILTER raw_data BY id is not null;
-- To dump the data from PIG Storage to stdout
--dump raw_data; 

filter_data = FILTER filter_data_question BY postTypeId == 1;
...
STORE filter_data INTO 'hbase://posts_question' USING org.apache.pig.backend.hadoop.hbase.HBaseStorage('posts_data:postTypeId posts_data:acceptedAnswerId posts_data:creationDatepost_tags_eachdata posts_data:score posts_data:viewCount posts_data:ownerUserId posts_data:lastEditorUserId posts_data:lastEditDate posts_data:lastActivityDate posts_data:title posts_data:tags posts_data:answerCount posts_data:commentCount posts_data:favoriteCount posts_data:parentId posts_data:communityOwnedDate posts_data:closedDate posts_data:ownerDisplayName posts_data:lastEditorDisplayName');
